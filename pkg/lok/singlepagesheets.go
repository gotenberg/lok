...
// When it returns a path different from inputPath, that path is a temporary file
// the caller must remove once the conversion is done.
func resetCalcScrollPosition(inputPath string) string {
	// Resolve the extension to a literal so the temporary filename is never
	// derived from the (caller-controlled) input name.
	var ext string
	...
	if err != nil || !changed {
		// The common case: nothing was saved scrolled, or the workbook could
		// not be read as a zip. Either way, leave it untouched.
		return inputPath
	}
	...
	dst, err := os.CreateTemp("", "singlepagesheets-*"+ext)
	if err != nil {
		return inputPath
	}
	defer dst.Close()
	...
	_, err = dst.Write(out)
	if err != nil {
		// Best-effort cleanup of the half-written file; the conversion falls
		// back to the original workbook.
		_ = os.Remove(dst.Name()) //nolint:gosec // dst is from os.CreateTemp in the system temp dir, not a caller-controlled path
		return inputPath
	}
