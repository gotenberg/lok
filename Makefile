...
fmt: ## Format Golang codebase in Docker
	docker run --rm lok-testing sh -c 'go fix ./... && golangci-lint fmt && go mod tidy'
...
lint: ## Lint Golang codebase in Docker
	docker run --rm lok-testing golangci-lint run

.PHONY: lint-todo
lint-todo: ## Find TODOs in Golang codebase in Docker
	docker run --rm lok-testing golangci-lint run --no-config --enable godox
...
test-unit: ## Run unit tests in Docker
	docker run --rm lok-testing go test -race -count=1 -v ./pkg/lok/...

.PHONY: test-integration
test-integration: ## Run integration tests in Docker
	docker run --rm -e GODEBUG=asyncpreemptoff=1 lok-testing go test -tags integration -count=1 -v ./...
